...
flowchart TD
  %% ==========================
  %% SECCIÓN 1: Checkout + Create Payment
  %% ==========================
  subgraph S1[Checkout → Create Payment → Redirect]
    direction TD
    ...
    E --> F[Redirect cliente → callback_url\n(UI Kueski)]
  end

  %% ==========================
  %% SECCIÓN 2: Webhook + Idempotencia
  %% ==========================
  subgraph S2[Webhook (IPN) → Auditoría + Idempotencia]
    direction TD
    F --> G[Kueski → Webhook\nPOST dispatch=kueski.webhook\n{order_id,payment_id,status,...}]
    G --> G0[Auditoría + Idempotencia (pre-guards)\nINSERT IGNORE en cscart_kueski_webhook_events\nDB-level idempotency\nUNIQUE(payment_id,status)\nDuplicado → responder idempotente, sin tocar orden]
  end

  %% ==========================
  %% SECCIÓN 3: Guards (como nodos)
  %% ==========================
  subgraph S3[Guards del Webhook]
    direction TD
    G0 --> G1{Guard 1:\norder_id numérico > 0?}
    ...
    G2 -- Sí --> G3{Guard 3:\n¿Mapping válido en cscart_kueski_payments?\n(order_id ↔ payment_id)}
    G3 -- No --> R1
  end

  %% ==========================
  %% SECCIÓN 4: Status mapping → CS-Cart
  %% ==========================
  subgraph S4[Status mapping → Estados CS-Cart]
    direction TD
    G3 -- Sí --> S{status Kueski}
  ...
  %% ==========================
  %% SECCIÓN 5: PAYMENT_NOTIFICATION (UX)
  %% ==========================
  subgraph S5[PAYMENT_NOTIFICATION (UX)]
    direction TD
