...
flowchart TD
  %% ==========================
  %% SECCIÓN 1: Trigger admin
  %% ==========================
  subgraph R1[Admin → Estado R]
    direction TD
    A[Admin cambia estado de orden → R\n(Reembolso solicitado)]
  end

  %% ==========================
  %% SECCIÓN 2: Hook + Guards
  %% ==========================
  subgraph R2[Hook → Guards Refund]
    direction TD
  ...
  %% ==========================
  %% SECCIÓN 3: Llamada a Kueski
  %% ==========================
  subgraph R3[KueskiApi::refund()]
    direction TD
    G -->|PASA| K1[POST /api/refunds\nAuth: JWT HS256 (api_secret)]
  end

  %% ==========================
  %% SECCIÓN 4: Resultado (X / rollback)
  %% ==========================
  subgraph R4[Resultado → X o rollback]
    direction TD
    K1 -->|OK| OK1[Auditoría en cscart_kueski_refunds\nstatus=completed + refund_id\n(INSERT IGNORE)]
    OK1 --> X[CS-Cart: X (Reembolsado)\nfn_change_order_status(order_id,'X')]

    K1 -->|FAIL| ER1[Auditoría error en cscart_kueski_refunds\n(ON DUPLICATE KEY UPDATE)\nerror_code/message/details]
    ER1 --> RB[Rollback orden → old_status\n(P o C)]
  end
```
